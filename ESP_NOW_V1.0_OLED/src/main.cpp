...
void loop()
{
    ...
    if (fsmState == STATE_EDIT_PARAM) {
        ...
        switch(subIndex) {
            case 1: // Deadzone 
            {
                ...
                if (newDz < 0) newDz = 0;
                if (newDz > 2000) newDz = 2000;
                sysConfig.deadzone = (uint16_t)newDz;
                break;
            }
            case 2: // Invert X/Y
            {
                if (encDiff > 0) sysConfig.invertX = !sysConfig.invertX;
                else sysConfig.invertY = !sysConfig.invertY;
                break;
            }
            case 4: // Tx Rate
            {
            ...
            // Add case 3: Channel, case 6: LED Maps later
        }
    }
    else if (!inSubMenu)
    {
  ...
  { // Transition to press
    ...
    else if (fsmState == STATE_TARGET_SELECT || fsmState == STATE_SETTINGS)
    {
      ...
      else
      {
        ...
        {
          if (subIndex == 0) { // Add Target (AP Portal)
    ...
    else if (fsmState == STATE_HUD || fsmState == STATE_TELEMETRY)
    {
  ...
  if (currentMillis - lastSendTime >= sendInterval)
  {
    ...
    displayManager.update(&telemetryData, networkManager.getLastStatus(), fsmState, subIndex, displayTarget, sysConfig, "N/A");
  }
}
