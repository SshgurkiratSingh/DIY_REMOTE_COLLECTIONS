...
<!DOCTYPE HTML><html><head>
  ...
  <form action="/save" method="post">
    ...
    <input type="text" name="mac" pattern="^([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})$" placeholder="XX:XX:XX:XX:XX:XX" required><br>
        <label>Verify Key (0-255)</label><br>
        <input type="text" name="vkey" pattern="^([0-9]{1,2}|1[0-9]{2}|2[0-4][0-9]|25[0-5])$" placeholder="e.g. 42" required><br>
...
{
    if (server.hasArg("name") && server.hasArg("mac") && server.hasArg("vkey"))
    {
        String name = server.arg("name");
        String macStr = server.arg("mac");
        String verifyKeyStr = server.arg("vkey");

        char *endPtr = nullptr;
        long verifyKey = strtol(verifyKeyStr.c_str(), &endPtr, 10);
        if (endPtr == verifyKeyStr.c_str() || *endPtr != '\0' || verifyKey < 0 || verifyKey > 255)
        {
            server.send(400, "text/plain", "Invalid verify key (must be 0-255).");
            return;
        }
                        ...
                        &values[3], &values[4], &values[5]))
        {
            ...
            stMgr->addTarget(name.c_str(), mac, (uint8_t)verifyKey);
