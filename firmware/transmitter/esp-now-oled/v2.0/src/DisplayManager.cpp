...
uint32_t DisplayManager::computeDataHash(const struct_message *data, bool sendStatus, MenuState state,
                                         int subIndex, const char *targetName, const SystemConfig &config,
                                         const rx_message *rxData)
{
    // Create a hash to detect if display content has changed
    uint32_t hash = 0;

    // Core data changes
    hash ^= ((data->joyX << 16) | (data->joyY & 0xFFFF));
    hash ^= (data->potValue << 8) | (sendStatus << 0);
    hash ^= (data->toggle1 << 0) | (data->toggle2 << 1) | (data->push1 << 2) | (data->push2 << 3);

    // Configuration changes
    hash ^= (config.deadzone << 16) | (config.txRateHz << 8) | (config.rxEnabled << 0);
    hash ^= (config.invertX << 0) | (config.invertY << 1) | (config.ledMode << 2);

    // State changes
    hash ^= ((int)state << 24) | ((subIndex & 0xFF) << 16);

    // RX data changes
    if (rxData)
    {
        hash ^= (rxData->data1 << 16) | (rxData->data2 << 8) | (rxData->data3 << 0);
    }

    // Simple string hash for target name
    if (targetName)
    {
        uint32_t nameHash = 0;
        for (int i = 0; i < 16 && targetName[i] != '\0'; i++)
        {
            nameHash = nameHash * 31 + targetName[i];
        }
        hash ^= nameHash;
    }
...
{
    // Compute data hash and check if display needs updating
    uint32_t currentHash = computeDataHash(data, sendStatus, state, subIndex, targetName, config, rxData);

    // Only redraw if something has changed
    if (currentHash == lastDataHash && state == lastState && subIndex == lastSubIndex)
    {
        // Still update LEDs every iteration, but skip display update
        goto update_leds;
    }
...
update_leds:
    // Status LEDs - always update, even if display didn't change
    if (config.outType == OUT_LEDS)
    {
    ...
    // If outType == OUT_VIB_BUZZ, FeedbackManager will control these pins.
}
