...
void StorageManager::addTarget(const char *name, const uint8_t *mac, uint8_t verifyKey)
{
    ...
    sprintf(key, "tgt_key_%d", count);
    preferences.putUChar(key, verifyKey);
...
TargetNode StorageManager::getTarget(int index)
{
    ...
    if (index >= 0 && index < count)
    {
        ...
        sprintf(key, "tgt_key_%d", index);
        node.verifyKey = preferences.getUChar(key, 0);
    }
