...
const int BURST_SPEED_LIMIT = 185;
...
bool slowTurnEnabled = false;
bool invertControlsActive = false;
...
int mapJoystickSigned(uint16_t raw, int speedLimit)
{
  ...
  return map(mapped, -PWM_MAX, PWM_MAX, -speedLimit, speedLimit);
}
...
void applyDriveFromMessage(const struct_message &incomingData)
{
  ...
  dynamicScalingEnabled = incomingData.toggle1;
  slowTurnEnabled = incomingData.toggle2;
  invertControlsActive = incomingData.push2;
  ...
  const int mapLimit = dynamicScalingEnabled ? speedLimit : PWM_MAX;
  int throttle = mapJoystickSigned(incomingData.joyY, mapLimit);
  int steer = mapJoystickSigned(incomingData.joyX, mapLimit);

  // Toggle 2: Slow Turning (halves steering sensitivity)
  if (slowTurnEnabled)
  {
    steer /= 2;
  }

  // Push 2: Invert Controls (mirrors ONLY Y axis)
  ...
  Serial.print("Dynamic scale: ");
  Serial.println(dynamicScalingEnabled ? "ON" : "OFF");
  Serial.print("Slow turn mode: ");
  Serial.println(slowTurnEnabled ? "ON" : "OFF");
  Serial.print("Reverse driving: ");
  Serial.println(invertControlsActive ? "ON" : "OFF");
